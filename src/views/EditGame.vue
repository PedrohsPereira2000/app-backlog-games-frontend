...
export default {
  ...
  async created() {
    ...
    try {
      const response = await axios.post(`${process.env.VUE_APP_BACKEND_URL}/backlog/search`, { user_id: userId, id: gameId })
  ...
  methods: {
    ...
    async saveGame() {
      try {
        const resp = await axios.post(`${process.env.VUE_APP_BACKEND_URL}/backlog/update`, {"user_id": this.$route.params.user_id, "game": this.game})
        if (resp.status === 200) {
